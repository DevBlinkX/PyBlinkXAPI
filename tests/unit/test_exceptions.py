...
@responses.activate
def test_wrong_json_response(pyblinkxapi):
    responses.add(
        responses.GET,
        "%s%s" % (pyblinkxapi.root, pyblinkxapi._routes["portfolio.positions"]),
        body="{a:b}",
...
@responses.activate
def test_wrong_content_type(pyblinkxapi):
...
@responses.activate
def test_native_exceptions(error_type, message, pyblinkxapi):
    responses.add(
        responses.GET,
        "%s%s" % (pyblinkxapi.root, pyblinkxapi._routes["portfolio.positions"]),
