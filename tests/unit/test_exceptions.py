...
@responses.activate
def test_wrong_json_response(pyjmproapi):
    responses.add(
        responses.GET,
        "%s%s" % (pyjmproapi.root, pyjmproapi._routes["portfolio.positions"]),
        body="{a:b}",
...
@responses.activate
def test_wrong_content_type(pyjmproapi):
    rdf_data = "<rdf:Description rdf:about=''><rdfs:label>jmpro</rdfs:label></rdf:Description"
    responses.add(
        responses.GET,
        "%s%s" % (pyjmproapi.root, pyjmproapi._routes["portfolio.positions"]),
        body=rdf_data,
...
@responses.activate
def test_native_exceptions(error_type, message, pyjmproapi):
    responses.add(
        responses.GET,
        "%s%s" % (pyjmproapi.root, pyjmproapi._routes["portfolio.positions"]),
