...
try:
    ...
    margin_detail = blinkx.order_margins(order_param_single)
    ...
    margin_detail = blinkx.order_margins(order_param_multi)
    ...
    margin_amount = blinkx.basket_order_margins(order_param_basket)
    logging.info("Required margin for basket order: {}".format(margin_amount))
    # Compact margin response
    margin_amount_comt = blinkx.basket_order_margins(order_param_basket, mode='compact')
...
try:
    ...
    order_book_charges = blinkx.get_virtual_contract_note(order_book_params)
