...
"""
    ...
    API wrapper for JMPRO Trading API REST APIs.
...
class PyJmproAPI(object):
    """
    The JMPRO Trading API wrapper class.
    ...
    _default_root_uri = "https://api.jmpro.in/SmartApi"
    _default_login_uri = "https://mobuadeh.jmpro.in/login/connect/login"
    _default_timeout = 7  # In seconds

    # Jmpro connect header version
    jmpro_header_version = "3"
    blinkx_header_version = jmpro_header_version
    ...
    _routes = {
        ...
        # JMPRO-specific endpoints
        "optionchain": "/optionchain",
        "optionchain.expiry": "/optionchain/expirylist",
        "ledger": "/ledger",
        "trades.history": "/trades/{from_date}/{to_date}/{page}",
    }
    ...
    def __init__(self,
                 ...
                 disable_ssl=False):
        """
        Initialise a new JMPRO Trading API client instance.
        ...
        to the Jmpro Connect class initialisation for subsequent requests.
    ...
    def login_url(self):
        ...
        return "%s?api_key=%s&v=%s" % (self._default_login_uri, self.api_key, self.jmpro_header_version)
    ...
    def generate_session(self, request_token, api_secret):
        ...
        if resp.get("login_time") and len(resp["login_time"]) == 19:
    ...
    def place_order(self,
        ...
        for k in list(params.keys()):
            if params[k] is None:
                del (params[k])
        print("Place Order Params: {}".format(params))
    ...
    def option_chain(self, exchange, underlying, expiry, strike_price=None):
        """
        Retrieve option chain data for the given underlying.

        - `exchange` is the exchange name (e.g., "NFO", "BFO", "MCX")
        - `underlying` is the underlying symbol (e.g., "NIFTY", "BANKNIFTY")
        - `expiry` is the expiry date (string in format of yyyy-mm-dd)
        - `strike_price` is an optional strike price to filter (float)
        """
        params = {
            "exchange": exchange,
        ...
        return self._post("optionchain", params=params, is_json=True)

    def option_chain_expiry(self, exchange, underlying):
        """
        Retrieve the list of available expiry dates for option chain.

        - `exchange` is the exchange name (e.g., "NFO", "BFO", "MCX")
        - `underlying` is the underlying symbol (e.g., "NIFTY", "BANKNIFTY")
        """
        return self._post("optionchain.expiry", params={
            "exchange": exchange,
            "underlying": underlying,
        }, is_json=True)
    ...
    def ledger(self, from_date, to_date):
        """
        Retrieve ledger/funds statement for the given date range.

        - `from_date` is the start date (string in format of yyyy-mm-dd or datetime object)
        - `to_date` is the end date (string in format of yyyy-mm-dd or datetime object)
        """
        date_string_format = "%Y-%m-%d"
        from_str = from_date.strftime(date_string_format) if isinstance(from_date, (datetime.datetime, datetime.date)) else from_date
        to_str = to_date.strftime(date_string_format) if isinstance(to_date, (datetime.datetime, datetime.date)) else to_date
    ...
    def trade_history(self, from_date, to_date, page=1):
        """
        Retrieve historical trade data for the given date range.

        - `from_date` is the start date (string in format of yyyy-mm-dd or datetime object)
        - `to_date` is the end date (string in format of yyyy-mm-dd or datetime object)
        - `page` is the page number for paginated results (default 1)
        """
        date_string_format = "%Y-%m-%d"
        from_str = from_date.strftime(date_string_format) if isinstance(from_date, (datetime.datetime, datetime.date)) else from_date
        to_str = to_date.strftime(date_string_format) if isinstance(to_date, (datetime.datetime, datetime.date)) else to_date

        return self._get("trades.history", url_args={
            "from_date": from_str,
            "to_date": to_str,
            "page": page,
        })
    ...
    def _parse_instruments(self, data):
        ...
        for row in reader:
            row["instrument_token"] = row["instrument_token"]
    ...
    def _request(self, route, method, url_args=None, params=None, is_json=False, query_params=None):
        ...
        url = self.root.rstrip('/') + '/' + uri.lstrip('/')

        # Custom headers
        headers = {
            "X-JMPRO-Version": self.jmpro_header_version,
            "User-Agent": self._user_agent()
        }

        if self.access_token:
            # set authorization header in token apiKey:accessToken format
        ...
        if "json" in r.headers["content-type"]:
            ...
            # handle non-2xx HTTP status codes where error fields may be absent
            if not r.ok:
                message = data.get("message") or data.get("error") or "HTTP error {}".format(r.status_code)
                raise ex.GeneralException(message, code=r.status_code)
