...
"""
    ...
    Websocket implementation for JMPRO ticker
...
class JmproTickerClientProtocol(WebSocketClientProtocol):
    """Jmpro ticker autobahn WebSocket protocol."""
    ...
    def __init__(self, *args, **kwargs):
        ...
        super(JmproTickerClientProtocol, self).__init__(*args, **kwargs)
    ...
    def _loop_ping(self):
        """Ping method loop which sends ping every X seconds."""
        self._send_ping()
        self._next_ping = self.factory.reactor.callLater(self.PING_INTERVAL, self._loop_ping)

    def _loop_pong_check(self):
        """Pong check method loop which checks if last received pong is older than X seconds."""
        if self._last_pong_time:
            time_diff = time.time() - self._last_pong_time
            if self.factory.debug:
                log.debug("Checking last pong time : {} diff: {}".format(self._last_pong_time, time_diff))

            if time_diff > self.KEEPALIVE_INTERVAL:
                if self.factory.debug:
                    log.debug("Last pong was received {} seconds ago. Closing connection.".format(time_diff))
...
class JmproTickerClientFactory(WebSocketClientFactory, ReconnectingClientFactory):
    """Autobahn WebSocket client factory to implement reconnection and custom callbacks."""

    protocol = JmproTickerClientProtocol
    maxDelay = 5
    maxRetries = 10
    ...
    def __init__(self, *args, **kwargs):
        ...
        super(JmproTickerClientFactory, self).__init__(*args, **kwargs)
...
class JmproTicker(object):
    """
    The WebSocket client for connecting to JMPRO Trading API's streaming quotes service.
    ...
    ---------------
        ...
        # Initialise
        kws = JmproTicker("your_api_key", "your_access_token")
    ...
    Auto reconnection is enabled by default and it can be disabled by passing `reconnect` param while initialising `JmproTicker`.
    ...
    next retry interval will be increased exponentially. `reconnect_max_delay` and `reconnect_max_tries` params can be used to tweak
    the algorithm where `reconnect_max_delay` is the maximum delay after which subsequent reconnection interval will become constant and
    ...
    def _create_connection(self, url, **kwargs):
        ...
        self.factory = JmproTickerClientFactory(url, **kwargs)
...
BlinkXTicker = JmproTicker
BlinkXTickerClientProtocol = JmproTickerClientProtocol
BlinkXTickerClientFactory = JmproTickerClientFactory
